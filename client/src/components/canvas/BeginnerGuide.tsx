...
function WelcomeModal({ onClose, onStartTour }: { onClose: () => void; onStartTour?: () => void }) {
  ...
  const features = [
    ...
    { Icon: Wand2, color: "oklch(0.70 0.20 310)",
      title: "画布助手 · 一句话快速成片", desc: "对右下角助手说「做一个雨夜侦探短片，6 个镜头，竖屏」——自动写脚本、拆分镜、建角色出定妆照、逐镜建生成节点并连线、按场景分组排版；生成后说「按镜头表装配」自动合成成片（镜号排序·逐镜转场·配音对位），「运行全部」批量生成（先弹费用确认），「做个动态样片」零成本预览节奏，「把陈默的音色锁成温柔女声」一句话锁角色音色（角色卡 🎙 徽标可见），「给每个镜头配音」逐镜自动配音（锁定音色沿用·多角色分音色·已配跳过）。快捷设置可锁比例/总时长/模型/转场/对白语种（你的设置永远优先）；📎 参考图据图定风格、可直接作视频首帧或角色锁脸；@ 引用角色库、/ 唤起技能；先框选=只改选中、随时一键撤销。建/连/改画布不花钱",
      badge: "HOT" },
    ...
    { Icon: Bookmark, color: "oklch(0.65 0.20 140)",
      title: "ComfyUI 节点模板库", desc: "右键任意 ComfyUI 节点把全部参数（含提示词 / 工作流）存为共享模板，全员可调用；按节点外框颜色分类、可搜索 / 注释 / 重命名，点击即在画布快速新建带参节点。注意区分：「ComfyUI 自定义」是导入你自己 workflow JSON 的空白节点，「节点模板库」是共享预设——两者在添加节点菜单里都有独立入口，别混",
      badge: "NEW" },
    { Icon: Music, color: "oklch(0.70 0.18 340)",
      title: "AI 配乐 · 配音 · 音效 · 音频工具", desc: "音频节点接入 Suno / MiniMax 音乐生成、OpenAI / ElevenLabs / 本地 VoxCPM 配音与 ElevenLabs SFX 音效生成；对白支持「角色名：台词」多角色分音色 casting，逐段各自配音自动拼接成镜级音频；对助手说「把陈默的音色锁成温柔女声」一句话锁定角色音色、「给每个镜头配音」全流程免手动；新增「工具」类别：人声分离（拆人声/伴奏/鼓/贝斯/钢琴/吉他多轨）、翻唱（换风格重唱）、续写（从第 N 秒接写）、写歌词",
      badge: "NEW" },
