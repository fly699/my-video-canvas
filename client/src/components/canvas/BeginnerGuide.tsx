...
function WelcomeModal({ onClose, onStartTour }: { onClose: () => void; onStartTour?: () => void }) {
  ...
  const features = [
    ...
    { Icon: Music, color: "oklch(0.70 0.18 340)",
      title: "AI 配乐 · 配音 · 音效 · 音频工具", desc: "音频节点接入 Suno / MiniMax 音乐生成、OpenAI / ElevenLabs / 本地 VoxCPM 配音与 ElevenLabs SFX 音效生成；对白支持「角色名：台词」多角色分音色 casting，逐段各自配音自动拼接成镜级音频；新增「工具」类别：人声分离（拆人声/伴奏/鼓/贝斯/钢琴/吉他多轨）、翻唱（换风格重唱）、续写（从第 N 秒接写）、写歌词",
      badge: "NEW" },
