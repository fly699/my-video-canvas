...
budgetGuard: false, budgetMax: 20, autoGroup: false, autoTidy: false, autoFocus: false, summaryMode: "compressed",
  ...
  const applyChatResult = (r: AgentChatResult, opts?: { skipPreview?: boolean; skipBudget?: boolean }): { apply: ReturnType<typeof applyAgentOperations> | null; fetchIds: string[] } => {
    ...
    if (ops.length) {
      ...
      if (quickPrefs.autoGroup && createdIds.length >= 2) {
        const gid = useCanvasStore.getState().groupSelected(createdIds, `AI编排 · ${createdIds.length}节点`);
        if (gid) applied = [applied, "已自动编组"].filter(Boolean).join(" · ");
      }
      // 落地后自动聚焦本次（开关，默认关）：把视野框到本次新建的节点上——大画布里落地节点
      // 常在视口外，开此项即无需再手点「聚焦本次」。放在整理/编组【之后】，框的是最终位置。
      // setTimeout 让新节点先进 store/完成布局再 fitView（同「聚焦本次」按钮口径）。
      if (quickPrefs.autoFocus && createdIds.length) {
        const ids = [...createdIds];
        setTimeout(() => reactFlow.fitView({ nodes: ids.map((id) => ({ id })), padding: 0.3, duration: 400, maxZoom: 1.5 }), 80);
      }
    }
