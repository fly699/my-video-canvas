...
export function PromptLibraryPanel({ onClose }: { onClose: () => void }) {
  ...
  // Signatures (label + text) of prompts already in my library — used to mark
  // presets as「已加入」, block duplicate adds, and dedup imports. One shared
  // key builder (collision-free) so every caller uses the exact same form.
  const sigKey = (label: string, text: string) => JSON.stringify([label, text]);
  const mineSig = useMemo(() => new Set(list.map((it) => sigKey(it.label, it.text))), [list]);
  const isAdded = (label: string, text: string) => mineSig.has(sigKey(label, text));
  ...
  const favPrompt = (label: string, text: string, category: string) => {
    if (isAdded(label, text)) { toast.info(`「${label}」已在我的库中`); return; }
  ...
  const importJson = async (file: File) => {
    try {
      ...
      // Dedup against existing prompts (and within this batch) by label + text.
      const seen = new Set(mineSig);
      let n = 0, dup = 0;
      for (const p of arr) {
        if (!p.label?.trim() || typeof p.text !== "string") continue;
        const label = p.label.trim();
        const key = sigKey(label, p.text);
        if (seen.has(key)) { dup++; continue; }
        await createMut.mutateAsync({ label, text: p.text, category: (p.category || "通用").trim(), slot: p.slot ?? null, slotKind: p.slotKind ?? null });
        seen.add(key);
        n++;
      }
      if (n > 0) toast.success(`已导入 ${n} 条提示词${dup ? `（跳过 ${dup} 条重复）` : ""}`);
      else if (dup > 0) toast.info(`全部 ${dup} 条提示词已存在，无需重复导入`);
      else toast.error("未找到可导入的提示词");
      refresh();
            ...
            <div key={c.category} className="flex flex-col gap-1 flex-shrink-0">
              ...
              {c.items.map((p) => {
                ...
                <div key={p.label} className="flex items-center gap-2 px-2.5 py-2 rounded-lg flex-shrink-0" style={{ background: "var(--c-input)", border: "1px solid var(--c-bd2)" }}>
                  ...
                  {added ? (
                    <span className="flex-shrink-0 flex items-center gap-0.5" title="已在我的库中" style={{ fontSize: 10, padding: "3px 7px", borderRadius: 6, border: "1px solid var(--c-bd2)", background: "transparent", color: "var(--c-t4)" }}><Check className="w-3 h-3" /> 已加入</span>
                  ) : (
