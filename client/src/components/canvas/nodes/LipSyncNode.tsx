...
const accent = "oklch(0.62 0.20 220)";
const accentA = (a: number) => `oklch(0.62 0.20 220 / ${a})`;
  ...
  return (
    <BaseNode id={id} selected={selected} nodeType="lip_sync" title={data.title} minHeight={120} showHandles={false}>
      <div className="flex flex-col items-center gap-3 p-4 text-center">
        ...
        <div>
          <p style={{ fontSize: 12, fontWeight: 600, color: "var(--c-t2)", marginBottom: 4 }}>唇形同步</p>
          <div className="flex items-center justify-center gap-1.5">
            <Lock style={{ width: 10, height: 10, color: "var(--c-t4)" }} />
            <p style={{ fontSize: 10, color: "var(--c-t4)" }}>需要配置 Sync.so API Key</p>
          </div>
        </div>
        <p style={{ fontSize: 9, color: "var(--c-t4)", lineHeight: 1.6 }}>
          将音频与视频中人物口型精准同步，支持换音换语言
        </p>
