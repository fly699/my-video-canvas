...
export const OverlayNode = memo(function OverlayNode({ id, selected, data }: Props) {
  ...
  const abandonWait = () => {
    ...
    toast.info("已取消等待：节点已解锁。服务器上的叠加任务无法中止，其结果不会回填本节点", { duration: 6000 });
  };
  ...
  const handleProcess = () => {
    ...
    abandonedRef.current = false; // 新一轮处理：复位「放弃等待」标记
  ...
  // 只看持久化 status——handleProcess 会同步先置 "processing"；并上 isPending 会让
  // 「放弃等待」后节点解不开锁（请求仍在飞）。
    ...
    <BaseNode id={id} selected={selected} nodeType="overlay" title={data.title} minHeight={240}
      ...
      <div className="flex flex-col h-full p-3.5 gap-3 overflow-auto">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          {/* 处理中变为可点的「取消」（对齐 #143/合并节点） */}
          <button
            onClick={isProcessing ? abandonWait : handleProcess}
            title={isProcessing ? "放弃等待 / 取消叠加（服务器任务无法中止，其结果不会回填）" : undefined}
            className="nodrag flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-semibold transition-all"
            style={{
              background: isProcessing ? "oklch(0.62 0.20 25 / 0.12)" : `${accentColor.replace(")", " / 0.15)")}`,
              borderWidth: 1, borderStyle: "solid",
              borderColor: isProcessing ? "oklch(0.62 0.20 25 / 0.4)" : accentColor.replace(")", " / 0.4)"),
              color: isProcessing ? "oklch(0.62 0.20 25)" : accentColor,
              cursor: "pointer",
            }}
          >
            {isProcessing
              ? <><XCircle className="w-3 h-3" />取消（处理中...）</>
