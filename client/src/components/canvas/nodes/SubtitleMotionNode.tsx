    ...
    <>
        ...
        {tab === "edit" && (
          <>
            ...
            {shotSubsSource && (
              <button onClick={handleShotSubtitles} disabled={isTranscribing || isBurning}
                title="用合并节点装配时的逐镜对白 + 成片各段精确起点直接生成字幕：零识别错误、零成本"
                className="nodrag flex items-center justify-center gap-1.5 w-full py-2 rounded-lg text-xs font-medium transition-all"
                ...
                <Clapperboard style={{ width: 12, height: 12 }} />
                从镜头表生成字幕（对白确定性对位）
              </button>
            )}
    ...
    {isCreativeMode && (
      <InlineGenBar nodeId={id} visible={!!selected} width={440}>
        <div className="nodrag" style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 11.5, fontWeight: 700, color: "var(--c-t2)", whiteSpace: "nowrap" }}>动态字幕</span>
          <span style={{ fontSize: 10.5, color: "var(--c-t4)", flex: 1, minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>逐词动效 / 转录 / 烧录</span>
