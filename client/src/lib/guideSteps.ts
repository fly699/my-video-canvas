...
export const GUIDE_STEPS: TourStep[] = [
  {
    ...
    body: [
      ...
      "什么都不想学也没关系——下一步先认识「画布助手」：一句话就能全自动出片，其余功能之后慢慢摸索即可。",
  ...
  {
    // #293 置顶：画布助手是本应用招牌，新手导览第二步即详细介绍（原在剪辑器之后，
    // 新用户看到前就退出导览了）。
    id: "agent",
    chapter: "画布助手",
    icon: "🪄",
    title: "画布助手 · 一句话快速成片",
    body: [
      "本应用的招牌能力：在右下角助手窗说一句「做一个雨夜侦探短片，6 个镜头，竖屏」——它会自动写脚本、拆分镜、建角色并出定妆照、逐镜建好生成节点并全部连线、按场景分组排版，整条成片流水线一次成型。",
      "生成完各镜后再说「按镜头表装配」即自动合并成片（镜号排序、逐镜转场、配音对位）；「运行全部」一键批量生成（先弹费用确认，放心）；「做个动态样片」用关键帧图零成本预览节奏。",
      "配音也是一句话：「把陈默的音色锁成温柔女声」锁定角色音色（角色卡出现 🎙 徽标）；「给每个镜头配音」逐镜自动配音——锁定音色自动沿用、多角色分音色、已配的镜自动跳过。",
      "「快速设置」里可锁定画面比例、目标总时长、生图/视频模型、转场风格、对白语种、合并短镜省次数——你锁定的设置永远优先于 AI 自选。📎 附参考图它会据图定风格，还能把图直接用作视频首帧或角色锁脸；@ 引用角色库、/ 唤起技能。",
      "先框选节点再说话 = 只改选中的那几个；每次改动都可一键撤销。建 / 连 / 改画布本身不花钱，真正生成才消耗点数——尽管多用。",
    ],
    tip: "不知道某功能在哪就直接问它；节点报错也可以直接丢给它解释和修复。",
  ...
  },
  {
    ...
    body: [
      ...
      "顶部还有两个快捷磁贴：「导入工作流」向导（把 ComfyUI 工作流 JSON 变成画布节点）、「节点模板库」（打开共享预设工作流库，选一个即落带参节点）。注意「ComfyUI 自定义」磁贴是导入你自己 JSON 的空白节点，和「节点模板库」不是一回事。也可在画布空白处双击快速新建。",
    ],
  ...
  {
    id: "minimal-quicklook",
    chapter: "效率",
    icon: "🖼️",
    title: "极简显示 Alt+Q · 速览 Alt+W",
    body: [
      "创意模式按 Alt+Q 进入「极简显示」：所有节点只留预览框（无边框、极窄阴影），整张画布秒变看片墙——选片、对比、汇报都清爽；再按一次恢复标准显示。",
      "Alt+W「速览」则临时展开全部节点的左侧参考窗与顶部提示词窗，一眼看全整张画布用了哪些素材和文案，再按或几秒后自动复原。",
    ],
    keys: ["Alt", "Q"],
    target: null,
  },
