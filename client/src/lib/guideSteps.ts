...
export const GUIDE_STEPS: TourStep[] = [
  ...
  },
  {
    ...
    title: "画布助手：整张画布的总指挥",
    body: [
      "这是本应用最强的入口——用一句自然语言直接指挥画布：「做一个橘猫晒太阳的竖屏短片，3 镜头」会自动写脚本、拆分镜、建好整条节点链并连线；「把刚才那个视频画幅改成 9:16」「把 @小明 加进第 2 个分镜」这类修改也一句话完成。",
      "它能做的远不止建节点：批量改参数与模型、按语义整理布局、引用角色库 / 素材库、/ 唤起技能模板、解释报错并给修复建议；先框选节点再说话 = 只改选中的那几个，每次改动都可一键撤销。",
      "建 / 连 / 改画布本身不花钱（真正生成才消耗点数），放心多用。对话上下文落库，跨设备与清缓存都不丢。",
    ],
    tip: "把它当成会用整个应用的搭档：不知道某件事在哪个菜单里时，直接开口问它、让它替你做。",
