...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  // ── 聊天 ─────────────────────────────────────────────────────────────────────
  {
    ...
    content: [
      ...
      { type: "h3", text: "文件传输" },
      { type: "p", text: "服务器模式：浏览器通过**预签名直传**把文件直接上传到对象存储（绕过服务器请求体限制，支持超大文件），历史中可下载；上限由管理员配置，默认 200MB（可调至 5GB）。无服务器模式：文件经**流式分块加密**通过服务器中转、不落库，默认上限 1GB；大文件（>8MB）发送时可选择「不加密快速发送」以显著提速。" },
      { type: "tip", text: "需要传几个 GB 的超大文件？服务器模式的预签名直传最合适（不占服务器带宽与内存）；只需在「管理后台 → 聊天管理」把单文件上限调大即可。" },
      { type: "h3", text: "管理员" },
