...
export const HELP_SECTIONS: HelpSection[] = [
  ...
  // ── 视频任务节点 ────────────────────────────────────────────────────────────
  {
    ...
    content: [
      ...
      { type: "warn", text: "图生视频类模型（如 Kling 2.1、Wan 图生、Higgsfield DoP）必须提供参考图——连接图像生成 / 分镜节点或填写参考图 URL。视频生成通常需 30 秒至数分钟，任务提交后可继续操作其他节点，完成后自动更新。" },
    ],
  },
  ...
  // ── ComfyUI 自定义工作流节点 ────────────────────────────────────────────────
  {
    ...
    content: [
      ...
      { type: "tip", text: "系统会自动检测 CLIPTextEncode（提示词）、KSampler（步数/CFG/采样器）、CheckpointLoaderSimple（模型）、EmptyLatentImage（尺寸）、ControlNet / Upscale / IPAdapter / VAE / LoadImage 等节点的参数。在 image 参数里直接填公网图片 URL 也会在运行时自动上传到 ComfyUI。" },
    ],
  },
