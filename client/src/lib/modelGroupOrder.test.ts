...
describe("modelGroupOrder — 自建 LLM 必须置顶（否则被埋在 kie 列表最底，用户配了却看不到）", () => {
  ...
  it("即梦（本机 CLI）排在 Kie、Poyo 之前（用户明确要求）", () => {
    expect(modelGroupOrder("即梦")).toBeLessThan(modelGroupOrder("Kie"));
    expect(modelGroupOrder("即梦")).toBeLessThan(modelGroupOrder("Poyo"));
    // 但仍在内置 Manus/Forge 之后，不喧宾夺主
    expect(modelGroupOrder("Manus")).toBeLessThanOrEqual(modelGroupOrder("即梦"));
  });
});
