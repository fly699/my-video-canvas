  ...
  },
  {
    ...
    sections: [
      ...
      { id: "assistant-film", title: "一句话快速成片（招牌）", paragraphs: [
        ...
        "成片五口令：各镜生成完说「按镜头表装配」自动合并成片（镜号排序、逐镜转场、配音对位）；「运行全部」一键批量生成（先弹费用确认清单，勾选参与、实时算价）；「做个动态样片」用分镜关键帧图零成本预览节奏，满意再花钱生成视频；「把陈默的音色锁成温柔女声」一句话锁定角色音色——写入角色声音档案（角色卡出现 🎙 徽标，点击可解除），之后所有配音自动沿用；「给每个镜头配音」逐镜自动建配音工位并生成 TTS——多角色对白按「角色名：台词」分音色合成、已配的镜自动跳过防重复扣费、执行前弹计价确认，分镜工作流和排除分镜的提示词链（对白写成「角色名：台词」行）都支持，配好的音装配时自动对位进成片。",
