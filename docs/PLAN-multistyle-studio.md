...
>
> **新风格的创新不删减**：参考（LibLib 等）启发的新功能，凡属真能力者，一律实现为
> **共享能力 + 契约**，让 **pro 与新风格同时获得**——既不丢失创新，又满足同步。
> 即 pro 只「冻结既有布局/样式」，**不冻结功能**：新功能通过契约的 extras 区自动出现在 pro。
...
│  pro(既有布局不改,但经契约 extras 区自动获得新功能) / studio / simple。  │
│  每个皮肤用自己的组件+布局+CSS token 渲染同一份「契约」。皮肤永不含逻辑， │
│  只调用 Layer 1 的 store action。                                        │
│  ★ 控件库为 token 驱动：同一控件组件靠 data-ui/data-theme 自动换皮，      │
│    pro 与 studio 共用 —— 新字段一次声明、各皮肤同款逻辑+各自样式呈现。    │
└───────────────────────────────────────────────────────────────────────┘
```
...
- **pro 既有布局/样式不改**：legacy 手写参数区原样保留；studio 独立目录 `canvas/studio/` + CSS 由 `[data-ui]` 限定，互不渗透。
- **pro 不冻结功能**：在 pro 的 `BaseNode` 末尾挂一个**契约 extras 区**（`<ContractExtras>`：渲染契约里"非 legacy"的新 actions(进 `•••`/工具条) 与新 advanced 字段）。新功能落契约 → pro 与 studio 同时长出来，无需重写 pro legacy。
- **皮肤零逻辑** → 功能更新天然自动同步。
...
## 七、参考所长 → 新功能清单与归类（创新不删减，pro 同步获得）

把参考图启发的点分三类。**A 类**=真新能力，先在 Layer 1 实现，再经契约同时上 pro+studio；
**B 类**=契约声明的 action/参数，落契约一处即各皮肤皆有；**C 类**=纯展示，只是 studio 皮肤换法。
（落地前需逐项核对现状：很多"看似新"的能力项目可能已部分存在，应优先接现成、避免重复造。）

| 创新点（来源参考） | 类别 | 落点 / 现状核对 | 谁获得 |
|---|---|---|---|
| 扩图 Outpaint | A | 接现有 PostProcess/编辑；缺则新增 runner | pro(`•••`)+studio |
| 擦除 / 抠图 / 增强(HD) / 调整像素 | A | 增强/HD 可能已有；擦除/抠图按需新增 | 全皮肤 |
| 快速切分 2×2/3×3/4×4 | A | 新增切分 action（纯前端切图或后端） | 全皮肤 |
| 多角度 / 打光 / 全景 | A | 映射到出图变体能力（参数化 prompt/重生成） | 全皮肤 |
| 动作迁移 / 镜头组合(Lens Combo) | A | 接对应模型(Kling/Seedance…)能力；可能已有 video 能力 | 全皮肤 |
| 合规验证 | A | 新增校验 action（若业务需要） | 全皮肤 |
| 媒体「替换」输入 | B | 契约 action：换 referenceImage/输入媒体（复用上传+连线预填） | 全皮肤 |
| 组「整组执行 / 更新模板 / 解组」 | B | 接 `requestRun`/快照·模板库/`ungroup` | 全皮肤(组工具条) |
| 按量计费贴发送键(⚡/秒) | B/C | 接现有点数预估，作为契约 action 元数据展示 | 全皮肤 |
| Prompt/Result 切页 · ✓ 选中徽标 · 胶片条 · 带类型端口 | B/C | 契约 ports/meta + 皮肤渲染 | studio 优先，pro 可选 |
| 媒体优先布局 / 悬浮上下工具条 / 右检查器 / 命令条 / Advanced 折叠 / 分区大标题 / 左竖栏 / 小地图 / 语音输入·1× | C | 纯 studio 皮肤呈现；逻辑全复用 | studio(可选 simple) |

**原则**：A/B 类一旦做进契约，**pro 通过 `<ContractExtras>` 自动获得**（以 pro 自己的样式呈现，例如新 action 进 pro 工具栏的 `•••`、新参数进 pro 的高级区）；不会"因为 pro 没有就砍掉创新"，也不会破坏自动同步。每个 A 类能力作为**独立小 PR**先落 Layer 1（含单测/真机验证），再在契约登记一行——风险可控、可逐个上。
