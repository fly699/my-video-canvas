...
> 📌 部署视角的**环境变量汇总表**（含本机 Claude/GPT 订阅桥接、自动下载等所有近期服务端功能）在根目录 **`DEPLOY.md`**「近期新增的服务端功能与配置」；代码任务的逐步勾选清单在 **`docs/phase2-启用清单.md`**。环境变量统一写项目根目录 `.env`，改后需重启 Node。
...
### 安全边界：它到底能不能碰你的真实项目/服务器代码？（务必读懂）

代码任务每段会话跑在**一个一次性隔离临时工作区**（`os.tmpdir()` 下的 `superagent-code-*`），`cwd` 与 `--add-dir` 都限于此，且每段新会话首轮自动前置「沙箱边界说明」提示词约束它别越界。能不能碰工作区以外的文件，取决于**有没有放第二把钥匙**：

| 档位 | 能否读写工作区以外（你的项目/服务器代码） |
|---|---|
| **只设 `SUPER_AGENT_CODE_ENABLED=1`**（默认，不放 Shell） | **不能。** claude 的内置文件工具（Read/Edit/Write）被 `--add-dir` 锁死在临时工作区，够不到你的项目。节点里显示绿色「只读沙箱」。 |
| **加设 `SUPER_AGENT_CODE_ALLOW_BASH=1`**（放行 Shell） | **能（理论上）。** Shell 命令以**服务进程的 OS 用户权限**运行，`--add-dir` 管不住 spawn 出来的子进程——它可读写该用户能访问的任何路径（含你的项目）。危险命令（`rm`/`sudo`/`shutdown`/`dd` 等）被 commandPolicy 执行前拦截，但普通的读/拷/写不一定拦。节点里显示红色「已放行 Shell」警告。 |

**建议**：只想安全用 → **不要设** `SUPER_AGENT_CODE_ALLOW_BASH`；需要 Shell 又要强隔离 → 把整个应用跑在容器/VM 里让 OS 层兜底。截图里 claude「是不是你那个项目 C:\…」是它在**猜**（默认档它够不到），不代表它有访问权。
