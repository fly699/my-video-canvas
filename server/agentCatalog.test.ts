...
import { normalizeScene } from "./routers/canvas";

describe("#330b 拆镜产出口（脚本→AI 生成分镜 normalizeScene）对白同口径去重", () => {
  it("相邻重复行压缩；隔行复读/同行复读原样保留（绝不丢词）", () => {
    const s1 = normalizeScene({ dialogue: "陈默：走。\n陈默：走。\n林晚：好。" }, 5);
    expect(s1.dialogue).toBe("陈默：走。\n林晚：好。");
    const keep = "甲：住手！\n乙：放开我！\n甲：住手！";
    expect(normalizeScene({ dialogue: keep }, 5).dialogue).toBe(keep);
    expect(normalizeScene({ dialogue: "陈默：救命！救命！" }, 5).dialogue).toBe("陈默：救命！救命！");
  });
  it("空对白→undefined；其它字段（promptText/description）不受去重影响", () => {
    expect(normalizeScene({ dialogue: "" }, 5).dialogue).toBeUndefined();
    const s = normalizeScene({ description: "重复 重复", promptText: "line\nline" }, 5);
    expect(s.description).toBe("重复 重复");
    expect(s.promptText).toBe("line\nline");
  });
});
