...
describe("normalizeBridgeMcp（桥接 MCP 配置容错解析 + 缺省值）", () => {
  it("空/undefined → 全缺省（strict 默认 true）", () => {
    expect(normalizeBridgeMcp(undefined)).toEqual({ mcpConfig: "", skills: false, strict: true, permissionMode: "", allowedTools: "" });
    expect(normalizeBridgeMcp(null)).toEqual({ mcpConfig: "", skills: false, strict: true, permissionMode: "", allowedTools: "" });
  });

  it("MySQL 8 返回对象 → 原样取值", () => {
    const o = { mcpConfig: '{"mcpServers":{}}', skills: true, strict: false, permissionMode: "acceptEdits", allowedTools: "Read,Grep" };
    expect(normalizeBridgeMcp(o)).toEqual(o);
  });

  it("MariaDB 返回 JSON 字符串 → 解析后取值", () => {
    const s = JSON.stringify({ mcpConfig: "/etc/mcp.json", skills: true, strict: true, permissionMode: "", allowedTools: "" });
    expect(normalizeBridgeMcp(s)).toEqual({ mcpConfig: "/etc/mcp.json", skills: true, strict: true, permissionMode: "", allowedTools: "" });
  });

  it("坏字符串 → 全缺省", () => {
    expect(normalizeBridgeMcp("{ not json")).toEqual({ mcpConfig: "", skills: false, strict: true, permissionMode: "", allowedTools: "" });
  });
