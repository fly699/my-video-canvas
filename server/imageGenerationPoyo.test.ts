...
describe("buildPoyoImageInput — 统一模型图生图（多模态能力恢复，docs/poyo-image-api.md §八/速查表）", () => {
  ...
  // 2026-07 真实故障回归：默认管线（model 未传）走 legacy 别名 "gpt-image-2"，此前该
  // spec 漏配 edit → 画面推演/多角度宫格的参考图被静默丢弃，产物与源图完全无关。
  it("legacy 别名 gpt-image-2：有参考图必须切 gpt-image-2-edit 并携带 image_urls", async () => {
    const { model, input } = await buildPoyoImageInput(POYO_IMAGE_SPECS["gpt-image-2"], { ...refs(A), size: "21:9" });
    expect(model).toBe("gpt-image-2-edit");
    expect(input.image_urls).toEqual([A]);
    expect(input.size).toBe("21:9"); // 比例透传（推演产物继承源图画幅）
    // 无参考图仍走文生图基座 wire。
    expect((await buildPoyoImageInput(POYO_IMAGE_SPECS["gpt-image-2"], { prompt: "p" })).model).toBe("gpt-image-2");
  });

  it("legacy 别名与正式 poyo_ spec 的 edit 变体保持同步（防再次漏配）", async () => {
    expect(POYO_IMAGE_SPECS["gpt-image-2"].edit).toBe(POYO_IMAGE_SPECS.poyo_gpt_image.edit);
    expect(POYO_IMAGE_SPECS["seedream-4.5"].edit).toBe(POYO_IMAGE_SPECS.poyo_seedream.edit);
    expect(POYO_IMAGE_SPECS["flux-2-pro"].edit).toBe(POYO_IMAGE_SPECS.poyo_flux.edit);
    expect(POYO_IMAGE_SPECS["flux-2-flex"].edit).toBe(POYO_IMAGE_SPECS.poyo_sdxl.edit);
  });
});
