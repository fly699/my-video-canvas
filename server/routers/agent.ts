...
/** prompt caching：把完整 system 切成「可缓存静态前缀」+「易变尾部」两段。纯函数，便于单测。
 *  - 静态前缀 = 结构上「# 当前画布」之前的目录/模型清单 + 「# 输出要求」起的规则段（跨轮不变，可缓存）。
 *  - 易变尾部 = 「# 当前画布」到「# 输出要求」之间的画布摘要/角色/偏好/人设/附件 + 追加的工作流经验。
 *  用结构标记切分（不重排大段规则文本，零笔误风险）；标记缺失/顺序异常时回退单块（不缓存，经验补回，
 *  systemVolatile 返回空串表示「只发一个 system 块」）。 */
export function splitSystemForCache(system: string, experienceSection: string): { systemStatic: string; systemVolatile: string } {
  const canvasMark = "\n\n# 当前画布\n";
  const rulesMark = "\n\n# 输出要求\n";
  const ci = system.indexOf(canvasMark);
  const ri = system.indexOf(rulesMark);
  if (ci >= 0 && ri > ci) {
    return {
      systemStatic: system.slice(0, ci) + system.slice(ri),           // 目录/模型清单 + 输出规则
      systemVolatile: system.slice(ci, ri) + experienceSection,       // 当前画布/角色/偏好 + 工作流经验
    };
  }
  return { systemStatic: system + experienceSection, systemVolatile: "" }; // 兜底：单块不缓存，经验补回
}
...
- 你只负责把工作流搭好并填好参数；是否触发生成由用户在画布上确认。`;
      ...
      const userContent: MessageContent[] | string = attachments.length
        ...
        : input.message;
      // prompt caching：把 system 切成「可缓存静态前缀」（目录/模型清单/输出规则——跨轮不变）
      // 与「易变尾部」（画布摘要/角色库/工作流经验/偏好/人设/附件——逐轮变）两个 system 块。
      // kieLLM 只对前缀打 cache_control：多轮对话与「自愈重试」间前缀命中缓存，省 TTFT 与费用；
      // 尾部不缓存，故它的变动不会让整个前缀缓存失效（否则会因 graphSummary 每轮变而白付缓存写入溢价）。
      // 用结构标记在运行时切分（见 splitSystemForCache），避免重排大段规则文本引入笔误。
      const { systemStatic, systemVolatile } = splitSystemForCache(system, comfyExperienceSection);
      const messages: Message[] = [
        { role: "system", content: systemStatic },
        ...(systemVolatile ? [{ role: "system" as const, content: systemVolatile }] : []),
          ...
          try {
            ...
            if (Array.isArray(parsed.operations)) {
              ...
              for (const o of parsed.operations) {
                const r = sanitizeOperationDetailed(o, { comfyOnly: input.comfyOnly, validTemplateIds, allowSuperAgent });
