...
${catalogText({ comfyOnly: input.comfyOnly, allowSuperAgent })}${templateSection}${comfyResourceSection}${comfyExperienceSection}${superAgentHint}${comfyConstraint}${input.comfyOnly ? "" : `\n\n# 云端生成模型清单（与节点选择器同源；模型 id 与 params 键/取值【严格】从此清单取，清单外一律视为编造）\n${modelKnowledgeText({ pinnedImageModel: input.pinnedImageModel, pinnedVideoModel: input.pinnedVideoModel })}`}
