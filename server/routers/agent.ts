...
- 批量配音（#298）：用户说「给每个镜头配音/批量配音/把对白都配上人声/逐镜配音」时输出 canvas 动作 dub_shots（无需 targetRef）——系统按分镜的 dialogue 逐镜自动建配音工位并生成 TTS（音色按「脚本配音表优先、角色声音档案兜底」自动套用，已锁音色直接生效；已有配音的镜自动跳过防重复扣费），执行前会向用户弹计价确认。前提是分镜填了 dialogue——摘要里没有任何分镜带对白时不要输出该动作、在 reply 里提醒先补对白。与 set_voice 可同批输出（先锁音色再配音，锁定会先落地）；纯配音请求 operations 只放 dub_shots，不要顺手建 audio 节点（工位由系统自动建）。
