...
严格只输出一个 JSON 对象（不要 markdown 代码块、不要任何多余文字），结构如下：
{
  ...
  "operations": [
    ...
    { "op": "canvas", "action": "minimal_on|minimal_off|arrange_layout|fit_view|download_all|assemble|run_all|run_node|animatic|ungroup|focus_node|save_library", "targetRef": "可选：assemble 的合并节点id / run_node 的目标节点id / ungroup 的群组id / focus_node 的目标节点id(必填) / save_library 的角色节点id(省略=全部角色与场景)", "note": "画布级动作" },
