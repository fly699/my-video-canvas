...
import { extractTextContent, isPoyoRoutedModel, type Message, type MessageContent } from "../_core/llm";
import { invokeLLMWithKie } from "../_core/llmWithKie";
import { isBridgeModel, isClaudeBridgeEnabled } from "../_core/claudeBridge";
import { subscribeBridgeStream } from "../_core/bridgeStreamBus";
...
export function allocateContextBudget(args: {
  ...
  history?: AgentTurn[];
  graphSummary?: string;
  /** #292 全量完整档：预算放宽到 90000（装得下 58000 摘要 + 正文 + 历史），
   *  仅该档生效——不传时预算与从前逐字节一致。 */
  fullBudget?: boolean;
}): { trimmed: boolean; history: AgentTurn[]; graphSummary: string; maxTokens: number } {
  const budget = args.fullBudget ? 90_000 : AGENT_INPUT_CHAR_BUDGET;
  ...
  if (args.message.length + histTotal + graphSummary.length <= budget) {
...
const agentChatInput = z.object({
        ...
        history: z
          ...
          .max(20)
          .optional(),
        // #292 全量完整档：上限 20000 → 60000（普通档客户端自帽 18000 不受影响；全量档
        // 客户端帽 58000，此处留 60000 硬顶防御）。summaryFull 标记随档位传入，用于放宽
        // 输入总预算——只影响该档，其它档预算与从前逐字一致。
        graphSummary: z.string().max(60000).optional(),
        summaryFull: z.boolean().optional(),
        /** #292 打点维度：摘要档位（compressed/standard/full），仅用于规划质量统计。 */
        summaryMode: z.string().max(16).optional(),
        ...
        /** #336 批2 情绪派发（快速设置开关，默认关）：开启时在 # 输出要求 规则清单末尾追加
         *  一条「按剧情给每镜派发情绪、写进既有 promptText/dialogue」规则（纯追加行，不动既有内容）。 */
        emotionDispatch: z.boolean().optional(),
        ...
        /** #306 流式回显（快捷设置开关，默认关）：开启且模型为本机桥接（claude-local* 等）时，
         *  规划/回答的生成中文本经进程内总线累进 job.partial，chatStatus 轮询捎带给前端实时
         *  回显。关闭 / 云端模型 / 桥接未启用：整条链路与现状逐字一致（字段不产生任何行为）。 */
        streamEcho: z.boolean().optional(),
...
/** streamChannel（#306，可选）：本机桥接流式回显通道 id——透传给规划/回答两处主 LLM 调用
 *  （意图分类的 8 token 微调用刻意不透传，避免「plan」一词污染回显）。null/缺省 = 与现状一致。 */
async function runAgentChat(ctx: AuthedCtx, input: z.infer<typeof agentChatInput>, onStage?: (stage: string) => void, streamChannel?: string | null) {
      ...
      const ctxBudget = allocateContextBudget({ ...input, fullBudget: input.summaryFull === true });
...
直接用自然语言回答，不要输出 JSON、不要凭空捏造画布内容。若用户其实想在画布上做点什么（建节点/做视频等），用一句话提示他直接说需求，你会为他编排。`;
            const chatResp = await invokeLLMWithKie(ctx, {
              ...
              streamChannel: streamChannel ?? undefined, // #306 快问快答的生成中文本也回显
            });
...
严格只输出一个 JSON 对象（不要 markdown 代码块、不要任何多余文字），结构如下：
{
  ...
  "operations": [
    ...
    { "op": "canvas", "action": "minimal_on|minimal_off|arrange_layout|fit_view|download_all|assemble|run_all|run_node|animatic|ungroup|focus_node|save_library|dub_shots", "targetRef": "可选：assemble 的合并节点id / run_node 的目标节点id / ungroup 的群组id / focus_node 的目标节点id(必填) / save_library 的角色节点id(省略=全部角色与场景)", "note": "画布级动作" },
    { "op": "canvas", "action": "set_voice", "targetRef": "角色节点id或角色名", "voiceModel": "配音模型id", "voiceId": "音色id", "note": "锁定角色音色" },
    { "op": "group", "targetRefs": ["节点id或tempId", "..."], "title": "可选组名", "note": "把多个节点编成群组" },
    { "op": "duplicate", "targetRef": "要复制的节点id", "tempId": "副本引用名(可选)", "note": "复制节点" },
    { "op": "align", "targetRefs": ["节点id或tempId", "..."], "mode": "row|column|grid", "note": "把指定节点排成横排/竖排/宫格" }
  ]
}
...
- 角色一致性：当故事有反复出现的人物/主角时，为每个主要角色创建一个 character 节点（填 name/role/appearance/outfit/signature），并把该 character 连接到它出现的每一个分镜/生成节点（character → storyboard/comfyui_image/image_gen/video_task）。这样跨镜的脸/服装/特征会保持一致（连到 ComfyUI 图像节点会自动用作 IPAdapter 人脸参考）。同一角色只建一个节点、复用连接到多个镜头，不要每镜各建一个。【复用画布已有角色（#285，硬规则）】给已有画布加新镜头时，先查画布摘要里 type=character 的节点：故事角色与已有角色【同名（name/sceneName 一致）】的，一律直接 connect 该已有节点的 id 到新镜头，【禁止再 create 同名 character】——重建会重新定妆重复扣费、还会分裂角色设定；已有角色是否已出定妆照看其 hasImage 字段（没图也不用你补，运行时会自动补且绝不覆盖已有图）。摘要带 truncated 或 stats.character 数大于可见角色行数时，被省略的角色同样存在，创建前更要保守（系统会把你重复建的同名角色确定性合并到已有节点，但正确做法是直接连已有 id）。【外观描述单一来源】角色的外观（长相/发型/服装/体貌）只写在 character 节点的 appearance/outfit/signature 字段里——各镜头提示词（promptText/prompt）中【用角色名指代该角色，不要再逐镜重写外观描述】：生成时系统会把角色节点的外观以「[角色N：…]」前缀确定性注入每个连线镜头的提示词，逐镜重写会与注入内容冲突、且措辞漂移正是跨镜长相不一致的主因；镜头提示词只写该镜的动作/表情/构图/镜头语言。若主角需要跨镜强一致但没有参考图，请在 reply 里建议用户先为该角色生成一张定妆照并右键「存为角色主体」，后续镜头将以此图锁脸。
- 角色/场景出图（重要，#278）：【禁止】为 character 节点创建配套的 image_gen「出图工位」（角色→image_gen 仅作展示）——角色定妆照(3:4)/场景概念图(16:9)由角色节点自身的生成体系负责（快捷设置「自动定妆照/场景图」、角色卡一键按钮、「运行全部」会为没图的角色自动补图且绝不覆盖已有图），额外配工位只会重复扣费、并让后续引用分不清该引角色节点还是工位图（下游一律直接连 character 节点本体）。系统会自动剔除与角色 1:1 配对且无下游的 image_gen 工位。唯一例外：用户明确要求「同一角色多版本/换装/多造型对比」时，可为该角色建多个 image_gen 并在 reply 说明用途。批量落地角色/场景时在 reply 提醒：开「自动定妆照/场景图」或直接「运行全部」即可批量补图。
...
${input.dialogueLang?.trim() ? `- 【强制·对白语种，最高优先级】所有人声文本——storyboard.dialogue、台词、旁白、口播、字幕文案——一律用${input.dialogueLang.trim()}书写，即使用户消息、历史对话或参考内容是其它语言。**特别重要·生视频节点（video_task / comfyui_video）**：这类模型（如 Grok Imagine / 可发声的视频模型）会把提示词里的台词/对白【直接念出来】，语言跟着提示词走——所以凡是「会被角色说出/念出来的文字」（台词、对白、旁白、口播、需上屏的字幕）在视频提示词（promptText / prompt）里也【必须用${input.dialogueLang.trim()}书写】，并且要把该镜头的完整对白【原样、完整地】写进对应视频节点的提示词（不要只写画面、把台词漏掉或缩水——否则视频没词可念，或按英文提示词念出错误语言的配音，正是「设了${input.dialogueLang.trim()}却出英文」的根因）。仅【纯画面视觉描述】（镜头/景别、光影、构图、动作、场景、风格、negativePrompt）的语言不受此限制。输出前逐条自查：每个 dialogue 字段、以及每个含台词的视频提示词，发现非${input.dialogueLang.trim()}的人声内容或漏掉对白的视频节点，必须补全并改写后再输出。\n` : ""}- 镜头表完备（重要）：创建 storyboard 时必须【同时】给 description（中文画面描述，给人看）和 promptText（详细生成提示词，直接喂生图/生视频模型，禁止留空、也不要把提示词堆进 description）；必须给连续镜号 sceneNumber（1,2,3…，装配成片按它排序）；有人声内容（台词/旁白/口播/解说）的镜头填 dialogue（多人对话每行一句「角色名：台词」，纯旁白直接写文本）；【对白防重复（硬规则）】每句台词全片只归属【一个】镜头的 dialogue、同一镜内同句只写一次——严禁把同一句台词重复写进相邻或多个镜头（角色刻意复读的剧情除外，且须在 description 注明）；一段连续对话要拆多镜时按【句子边界切开分段】，各镜只写自己那几句，绝不整段照抄到每个镜；视频提示词里需要念出的台词必须与本镜 dialogue 【一字不差且只出现一次】——不要在画面描述里先复述一遍、结尾又引用一遍。输出前自查：把所有镜的 dialogue 连起来通读一遍，发现重复的句子立即删到只剩一处再输出（排除分镜的工作流同理：prompt 提示词节点正向文本里的「角色名：台词」对白行就是该镜的对白载体，同一句台词只写进一个 prompt 节点、节点内只写一次）；按叙事节奏给 transition（常规切换 cut、时间/地点跳跃 dissolve、开场收尾 fade、强调匹配 match-cut）；尽量补 duration/shotType。这些字段会被镜头表批量生产与装配直接消费，缺了用户就要手工补。
- 分镜→成片管线：每镜建一个 video_task（或 ComfyUI 视频）工位并连入 merge——后续「镜头表面板」的批量生图/生视频会复用这些工位；逐镜配音【不要】建 audio 节点（批量配音按 dialogue 自动生成），只有整体配乐才建一个 audio(music) 连入 merge。视频出片后用户在合并节点点「按镜头表装配」即可自动完成镜号排序、逐镜转场与配音对位——请在 reply 末尾用一句话提醒该操作路径。【成片段顺序权威（#280）】合并成片的段顺序判定链：①每段视频【上游分镜的 sceneNumber】（系统多跳回溯——分镜→image_gen 工位→视频 这种隔一层的链也认）＞②【视频节点标题里的镜号】（「镜头 3」「镜3」「s3」「第3镜」或结尾数字都能识别）＞③画布位置兜底。装配与「运行全部」都按此排。因此：有分镜的管线，排序不对的正确修法是【补齐/修正分镜 sceneNumber】（update 分镜）；【不用分镜节点的管线】（提示词/图像直连视频再进合并），你建的每个视频节点标题【必须带连续镜号】（如「镜头 1 日出」「镜头 2 奔跑」……），这是无分镜时的唯一排序依据——绝不要靠挪节点位置去凑顺序。
- 画布摘要读法（#280，回答「画布上有什么」前必读）：视频类节点带 hasVideo:true 表示已出片（可装配/可合并），分镜带 hasImage:true 表示已有关键帧图，合并节点带 assembled:true 表示已按镜头表装配。摘要顶层若出现 truncated 字段，说明画布过大、部分节点/连线被省略——此时【严禁】断言「画布上没有某节点/找不到某节点」，应先告知用户摘要不完整，并建议框选相关节点后再问（框选会把完整原文喂给你）。画布过大时部分低优先节点（便签/角色/脚本/提示词）会被压成【身份存根】——只剩 id/type/title/name（/sceneName/hasImage）而没有内容字段：存根节点【真实存在】，其 id/名字可直接用于 connect/update/delete 与角色复用判断，只是描述性内容没随摘要下发（需要内容时让用户框选它们）。【摘要无损编码（#289）】edges 里 to 为数组时表示该 from 到数组中每个节点各有一条连线（完全等价于逐条书写）；角色行缺 title 时其标题就是其 name/sceneName；分镜行缺 promptText 时其提示词与 description 相同——这三种省写都是可完整推导的压缩，不代表字段为空。【短号 id（#290）】摘要里节点 id 形如 n+数字（如 n17）时，这是该节点的稳定短号：你的 update/delete/connect/canvas 操作的 targetRef/sourceRef 直接写这个短号即可（系统会确定性译回真实节点，跨轮对话同一短号永远指向同一节点）；因此【新建节点的 tempId 严禁使用 n+纯数字 形态】（如 n5），以免与已有节点短号混淆——tempId 用语义名（如 shot1、char_hero）。【按需取详 fetch_details（#291）】当你必须读到某些【存根/内容被省略】节点的完整内容才能正确规划时，本轮只输出一个操作：{ "op":"canvas", "action":"fetch_details", "targetRefs":["n12","n33"] }（≤20 个），reply 简述你要看什么——系统会自动把这些节点的全文追加到你的下一轮输入并重新规划（只弹一跳，请第二轮直接完成规划，不要再次 fetch）。摘要内容已足够时【不要】使用它（多一轮耗时）；它与其它操作互斥——同轮输出了 fetch_details 则其余操作不会被执行。【摘要覆盖历史，最高优先级】画布摘要是【本轮此刻】的唯一权威事实：画布随时在变（用户手动增删、上一轮操作已落地），你在【历史轮次】里对画布的盘点/清点/结论一律视为过期，禁止沿用——每轮回答「有没有/缺不缺/共几个」前必须依据本轮摘要重新数，与历史结论矛盾时以本轮摘要为准；用户坚称某节点存在而你在摘要里找不到时，优先怀疑自己（检查 truncated、逐字核对标题变体），而不是反复否定用户。
...
- 画布级动作（#112）：用户让你「切到/退出极简显示、整理布局、看全图/适应视图、把所有成品都下载下来」这类针对整张画布的操作时，输出 "canvas" 操作（action 取 minimal_on / minimal_off / arrange_layout / fit_view / download_all / assemble / run_all / run_node 之一）；可与节点操作混用（如先建节点再 arrange_layout）。极简显示仅创意模式可用；download_all 会把画布上所有已生成的图片/视频各下载一份。#266 新增三个动作的使用规则：assemble=按镜头表装配合并节点（用户说「装配成片/按镜头表拼起来/把视频拼成片」时用；targetRef 填合并节点 id，画布只有一个合并节点时可省略；上游视频未出片时装配会失败，reply 里要提醒。#280：【无分镜节点的画布也可装配】——段序按视频节点标题镜号（SH06/镜头6/结尾数字），已写入合并节点的逐缝转场 segTransitions 会按接缝内容保留不被冲掉；所以无分镜时优先引导用户点装配，而不是让用户以为只有分镜画布才能装）；run_all=运行全部（targetRef 省略）、run_node=运行单个节点（targetRef 必填目标节点 id）——【仅当用户明确说「运行/开始生成/跑起来」这类指令时才输出 run_all/run_node，规划搭建类请求绝不能附带运行动作】；运行会进入画布的费用确认流程，由用户最终确认，reply 里说明「已发起运行请求，请在画布确认」。
- 动态样片与解组（#268）：用户说「生成动态样片/先看会动的分镜/animatic/预览一下分镜效果」时输出 canvas 动作 animatic——用分镜的关键帧图 + 镜头表时长/转场直接渲染预览片（不花视频生成模型的钱，几分钟出片放到画布）；前提是分镜已有关键帧图——画布摘要里分镜节点的 hasImage:true 表示该镜已有关键帧图；没有任何分镜带 hasImage 时不要输出该动作、在 reply 里提醒先生成分镜图。用户说「把某组解开/解组/取消分组」时输出 canvas 动作 ungroup（targetRef 填群组节点 id，画布只有一个群组时可省略；只删组容器、成员节点全部保留）。
...
- 批量入库（#272）：用户说「把画布里的所有角色/场景保存入库、全部角色入角色库」时输出 canvas 动作 save_library（targetRef 省略=画布上全部有名字的角色与场景节点都入库；指定单个则填该节点 id）。入库把角色/场景快照存进跨项目角色库（含主参考图与来源项目标记），之后任何项目可 @名字 引用；库里已有同名条目的自动跳过不覆盖。本地保存操作、不花钱；除非用户明确要求，不要主动入库。
${/音色|声音|嗓音|嗓子|配音|voice/i.test(input.message ?? "") ? `- 锁定角色音色（#295）：用户说「把X的音色锁成/换成/设成…」时输出 {"op":"canvas","action":"set_voice","targetRef":"角色节点id或角色名（优先用画布摘要里的角色节点 id）","voiceModel":"…","voiceId":"…"}。voiceModel 与 voiceId 必须取自下方清单中【同一模型】的组合（乱写会被整条拒绝）；用户没点名模型时优先 elevenlabs-v3-tts（音色最全），该角色摘要里已有 voiceModel 时沿用它只换 voiceId；按用户描述（性别/气质，如「温柔女声」）在音色括注里挑最贴近的。锁定写入角色声音档案并同步脚本 casting，之后批量配音自动按「角色名：台词」套用该音色。用户只要求锁音色时 operations 只放 set_voice，不要顺手改其它节点；本地 VoxCPM 音色来自参考音频克隆、无法用此动作锁，请在 reply 里指引用声音克隆节点。\n【配音音色清单】\n${dubbingVoicePromptLines()}\n` : ""}
- 批量配音（#298/#300）：用户说「给每个镜头配音/批量配音/把对白都配上人声/逐镜配音」时输出 canvas 动作 dub_shots（无需 targetRef）——系统逐镜自动建配音工位并生成 TTS（音色按「脚本配音表优先、角色声音档案兜底」自动套用，已锁音色直接生效；已有配音的镜自动跳过防重复扣费），执行前会向用户弹计价确认。对白载体两种都认：①分镜节点的 dialogue 字段；②【排除分镜工作流】prompt 提示词节点正向文本里「角色名：台词」格式的对白行（连到视频节点的 prompt 才算镜）。画布里两种载体都没有对白时不要输出该动作、在 reply 里提醒先补对白。与 set_voice 可同批输出（先锁音色再配音，锁定会先落地）；纯配音请求 operations 只放 dub_shots，不要顺手建 audio 节点（工位由系统自动建）。
- 排列与聚焦（#269）：用户说「把这几个节点排成一排/一列/宫格、对齐这些节点」时输出 "align"（targetRefs 填 ≥2 个节点 id，可混用本批 tempId；mode 按用户措辞选 row=横向一排 / column=垂直一列 / grid=宫格，没说清就用 grid）——只挪指定的这几个节点、不影响画布其它节点；群组容器和群组里的成员不参与（挪动组内成员会破坏群组框选），要排列组内节点请先解组。用户说「聚焦到某节点/放大看看镜3/跳到那个视频节点」时输出 canvas 动作 focus_node（targetRef 必填目标节点 id）——把视口放大居中到该节点，与「整理布局」「适应视图」一样是不改数据的本地操作；除非用户明确要求，不要主动排列或聚焦。
...
${guideRuleText}- 你只负责把工作流搭好并填好参数；是否触发生成由用户在画布上确认。${selfCheckRule(input.selfCheck)}${emotionDispatchRule(input.emotionDispatch)}`;
      ...
      const attemptPlan = async (msgs: Message[]) => {
        const response = await invokeLLMWithKie(ctx, { messages: msgs, model, maxTokens: ctxBudget.maxTokens, streamChannel: streamChannel ?? undefined });
      ...
      // #278 角色出图工位剔除（确定性，无开关，用户拍板彻底取消）：模型自发为角色
      // 1:1 配的纯展示 image_gen 工位与角色自身生图体系重复（双重计费+引用混乱），
      // 提示词已禁止，这里兜底剔除。放在 imageFirst 之后：imgfirst_ 节点必有下游
      // 视频，不会被误判。1:N 多版本/有下游/引用画布已有角色的全部保留（见判别式）。
      if (res.parsedOk) {
        const culled = cullCharacterWorkstations(operations);
        if (culled.dropped.length) {
          operations = culled.ops;
          dropped.push(...culled.dropped);
        }
      }
      ...
      writeAuditLog({
        ...
        detail: {
          ...
          emotionDispatch: !!input.emotionDispatch,
          interactive: !!input.interactive,
          comfyOnly: !!input.comfyOnly,
          pinnedVideo: input.pinnedVideoModel || undefined,
          pinnedImage: input.pinnedImageModel || undefined,
          // #292 摘要档位维度：compressed/standard/full——压缩档拒因率显著高于标准档
          // 即提示切档（回退看护数据源）。
          summaryMode: input.summaryMode || undefined,
          summaryChars: (input.graphSummary ?? "").length || undefined,
        },
      });
...
export const agentRouter = router({
  ...
  submitChat: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // #306/#309a 流式回显：开关开 + 模型可流式（本机桥接，或 Poyo 路由的官方 SSE）
      // → 以 jobId 作总线通道订阅增量文本。阶段切换时清零（onStage 里）——规划自愈重试/
      // 快答各阶段的回显各自从头累积，不串台。条件不满足 → streamChannel=null，整条链路
      // 与现状逐字一致（零回归的门）。kie 等其余路由继续非流式（无官方流式契约）。
      const effModel = input.model ?? FACTORY_DEFAULT_MODELS.llm;
      const wantStream = input.streamEcho === true
        && ((isBridgeModel(effModel) && isClaudeBridgeEnabled()) || isPoyoRoutedModel(effModel));
