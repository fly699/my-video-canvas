...
直接用自然语言回答，不要输出 JSON、不要凭空捏造画布内容。若用户其实想在画布上做点什么（建节点/做视频等），用一句话提示他直接说需求，你会为他编排。`;
            const chatResp = await invokeLLMWithKie(ctx, {
              ...
              bridgeStreamChannel: streamChannel ?? undefined, // #306 快问快答的生成中文本也回显
            });
...
${guideRuleText}- 你只负责把工作流搭好并填好参数；是否触发生成由用户在画布上确认。${selfCheckRule(input.selfCheck)}`;
      ...
      const attemptPlan = async (msgs: Message[]) => {
        const response = await invokeLLMWithKie(ctx, { messages: msgs, model, maxTokens: ctxBudget.maxTokens, bridgeStreamChannel: streamChannel ?? undefined });
...
export const agentRouter = router({
  ...
  submitChat: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // #306 流式回显：开关开 + 本机桥接模型 + 桥接启用 → 以 jobId 作总线通道订阅增量文本。
      // 阶段切换时清零（onStage 里）——规划自愈重试/快答各阶段的回显各自从头累积，不串台。
      // 三条件任一不满足 → streamChannel=null，整条链路与现状逐字一致（零回归的门）。
      const effModel = input.model ?? FACTORY_DEFAULT_MODELS.llm;
      const wantStream = input.streamEcho === true && isBridgeModel(effModel) && isClaudeBridgeEnabled();
