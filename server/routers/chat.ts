...
export const chatRouter = router({
  ...
  joinRoom: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // 纵深防御：system: 前缀 或 createdBy=null 的系统/官方房，即使知道 roomId 也不能直接 join。
      if ((conv.dmKey ?? "").startsWith("system:") || conv.createdBy == null) throw new TRPCError({ code: "FORBIDDEN", message: "该房间为私有房间，无法直接加入" });
